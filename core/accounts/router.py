...
def account_register(request: schemas.UserRegistrationSchema, db: Session = Depends(get_db)):
    ...
    if user_obj:
        raise HTTPException(
            status_code=409, detail="user already exists you cannot create another with the same email")
...
def change_password(request: schemas.ChangePasswordSchema, db: Session = Depends(get_db), user_id: int = Depends(JWTBearer())):
