...
def post_detail(id: int, db: Session = Depends(get_db)):
    ...
    post_obj = db.query(models.PostModel).filter(models.PostModel.id == id).first()
    if not post_obj:
...
def post_create(request: schemas.PostSchema, db: Session = Depends(get_db)):
    post_obj = models.PostModel(title=request.title, content=request.content)
    db.add(post_obj)
    db.commit()
    db.refresh(post_obj)
...
def post_update(id: int, request: schemas.PostSchema, db: Session = Depends(get_db)):
    post_obj = db.query(models.PostModel).filter(models.PostModel.id == id)
