...
async def post_list(
    ...
    if search is not None:
        posts = posts.filter(
            models.PostModel.title.contains(search)
            | models.PostModel.content.contains(search)
        )
    ...
    posts = posts.all()
    results = [
        jsonable_encoder(schemas.PostResponse.from_orm(post)) for post in posts
    ]
    return JSONResponse(
        content={
            "page": page,
...
async def post_detail(id: int, db: Session = Depends(get_db)):
    post_obj = (
        ...
        .filter(
            models.PostModel.id == id, models.PostModel.is_published == True
        )
        .first()
    )
    if not post_obj:
    ...
    return JSONResponse(
        content=jsonable_encoder(schemas.PostResponse.from_orm(post_obj)),
        status_code=status.HTTP_200_OK,
    )
...
async def post_list(
    ...
    posts = posts.all()
    results = [
        jsonable_encoder(schemas.AuthorPostResponse.from_orm(post))
        for post in posts
    ]
...
async def post_detail(
    id: int, db: Session = Depends(get_db), user_id: int = Depends(JWTBearer())
):
    # raise HTTPException(status_code=500,detail="details are missing")
    post_obj = (
        db.query(models.PostModel)
        .filter(models.PostModel.id == id, models.PostModel.user == user_id)
        .first()
    )
    if not post_obj:
        raise HTTPException(status_code=404, detail="post not found")
